...
import com.fan.base.GeneralDao;
...
public abstract  class GeneralDaoImpl<T> implements GeneralDao<T> {
	...
	public Pagination<T> findPaginationByQuery(Query query, int pageNo, int pageSize) {
		if (query == null) {
			query = new Query();
		}
	...
	public int updateAllByQuery(Query query, Update update) {
		WriteResult writeResult = this.mongoTemplate.updateMulti(query, update, this.getEntityClass());
		if(writeResult != null){
	...
	public WriteResult removeByQuery(Query query) {
		return this.mongoTemplate.remove(query,this.getEntityClass());
	}
