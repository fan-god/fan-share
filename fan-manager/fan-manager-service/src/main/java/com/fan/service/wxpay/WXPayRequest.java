...
public class WXPayRequest {
    ...
    private String requestOnce(final String domain, String urlSuffix, String uuid, String data, int connectTimeoutMs, int readTimeoutMs, boolean useCert) throws Exception {
        ...
        StringEntity postEntity = new StringEntity(data, ConstantFan.CHARSET);
        httpPost.addHeader("Content-Type", "text/xml");
        httpPost.addHeader("User-Agent", FieldConstant.WxPay.USER_AGENT + " " + config.getMchID());
        httpPost.setEntity(postEntity);

        HttpResponse httpResponse = httpClient.execute(httpPost);
        HttpEntity httpEntity = httpResponse.getEntity();
        return EntityUtils.toString(httpEntity, ConstantFan.CHARSET);
    ...
    private String request(String urlSuffix, String uuid, String data, int connectTimeoutMs, int readTimeoutMs, boolean useCert, boolean autoReport) throws Exception {
        ...
        try {
            ...
            elapsedTimeMillis = System.currentTimeMillis()-startTimestampMs;
            log.warn("connect timeout happened for domainInfo {}", domainInfo);
            WXPayReport.getInstance(config).report(
                    uuid,
                    elapsedTimeMillis,
                    domainInfo.domain,
